...
Strategy.prototype.userProfile = function(token, tokenSecret, params, done) {
  var url = 'https://api.linkedin.com/v1/people/~:(id,first-name,last-name,picture-url,public-profile-url)?format=json';
  ...
  this._oauth.get(url, token, tokenSecret, function (err, body, res) {
    ...
    try {
      ...
      // The following profile fields are optional and only present if returned by LinkedIn
      if (json.emailAddress) { profile.emails = [{ value: json.emailAddress }]; };
      if (json.pictureUrl) { profile.photos = [{value:json.pictureUrl, type:"thumb"}]; };
      if (json.publicProfileUrl) { profile.profileUrl = json.publicProfileUrl; };
...
Strategy.prototype._convertProfileFields = function(profileFields) {
  ...
  var fields = ['id', 'first-name', 'last-name'];
